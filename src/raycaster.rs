use chaos_framework::{vec2, EventLoop, Renderer, Vec3};
...
impl Raycaster {
    pub async fn get_body_from_mouse(
        el: &EventLoop, 
        renderer: &Renderer,
        world: &mut World,
        ctx: &ViewportCtx,
    ...
    pub async fn get_world_pos_from_mouse(
        el: &EventLoop, 
        renderer: &Renderer,
        world: &mut World,
        ctx: &ViewportCtx,
        ...
        if let Some(pos) = phys_world.pos_raycast(origin, dir) {
            return Some(pos)
        } else {
            return None;
        }
